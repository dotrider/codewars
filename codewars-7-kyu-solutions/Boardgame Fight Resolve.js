...
function fightResolve(defender, attacker){
    ...
    const check = (s) => s.toLowerCase() === s? 'l':'u'
    
    if(check(defender) === check(attacker)) return -1
    ...
    let result = {
      ...
      'p':'k',
      'k':'a'
    }, att = attacker.toLowerCase(), def = defender.toLowerCase()
    
    return result[def] === att? defender: attacker ;
  } 